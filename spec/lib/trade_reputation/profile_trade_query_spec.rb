...
describe TradeReputation::ProfileTradeQuery do
  ...
  describe "summary" do
    ...
    it "excludes feedback received by another user" do
      ...
      expect(query(user: target_user)[:summary][:all_time][:total]).to eq(0)
    end
    ...
    it "scopes given_total to feedback the target user submitted as reviewer" do
      ...
      expect(query(user: target_user)[:summary][:given_total]).to eq(1)
    end
    ...
    it "excludes an unrelated reviewer's feedback from given_total" do
      ...
      expect(query(user: target_user)[:summary][:given_total]).to eq(0)
    end
  end
  ...
  describe "history" do
    it "returns only feedback received by the target user" do
      ...
      expect(query(user: target_user)[:feedbacks].size).to eq(1)
    end
    ...
    it "orders newest created_at first" do
      ...
      expect(query(user: target_user)[:feedbacks].map { |f| f[:created_at] }).to eq([newer.created_at, older.created_at])
    end
    ...
    it "exposes an opaque public id and display transaction reference" do
      feedback = create_feedback(reviewee: target_user, reviewer: reviewer)
      entry = query(user: target_user)[:feedbacks].first
      expect(entry[:public_id]).to eq(feedback.public_id)
      expect(entry[:transaction_reference]).to eq("TR-#{feedback.marketplace_transaction_id}")
    end
  ...
  describe "missing reviewer" do
    it "returns a nil reviewer when the referenced user no longer exists" do
      create_feedback(reviewee: target_user, reviewer: reviewer)
      reviewer.destroy
      expect(query(user: target_user)[:feedbacks].first[:reviewer]).to be_nil
    end
  end
  ...
  describe "pagination" do
    ...
    it "caps per_page at the maximum" do
      expect(query(user: target_user, params: { per_page: "500" })[:meta][:per_page]).to eq(100)
    end
